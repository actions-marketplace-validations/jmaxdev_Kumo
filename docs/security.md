...
| `allowed_domains` | Array | A whitelist of domains from which Kumo is allowed to download package tarballs, preventing lockfile registry poisoning. | `["github.com", "objects.githubusercontent.com", "registry.npmjs.org", "nodejs.org"]` |
...
* **Environment Variable Whitelisting & CI Secret Purging:** Across all operating systems, Kumo completely clears host environment variables before spawning scripts. In CI mode (`kumo ci`), Kumo explicitly purges all sensitive CI tokens (`GITHUB_TOKEN`, `AWS_SECRET_ACCESS_KEY`, `NPM_TOKEN`, `AZURE_CLIENT_SECRET`, `GCP_SERVICE_ACCOUNT`, `SSH_PRIVATE_KEY`, etc.) and any environment variable matching `SECRET`, `PASSWORD`, `PRIVATE_KEY`, `_TOKEN`, or `_CREDENTIALS` to prevent supply-chain data exfiltration.
* **AST Static Code Analysis:** Before executing scripts, `crates/security/src/ast.rs` performs static code analysis on JavaScript/TypeScript files, flagging suspicious patterns such as Base64 obfuscated payloads (`atob`, `Buffer.from(..., 'base64')`), raw socket IP connections (`/dev/tcp`, `nc`, `connect("1.2.3.4")`), and direct secret access (`process.env.GITHUB_TOKEN`).
