...
register_commands!(
    #[command(alias = "i", about = "Install dependencies from kumo.json or package.json")]
    install,
    
    #[command(alias = "a", about = "Add a new package to the project")]
    add,
    
    #[command(alias = "rm", alias = "un", alias = "uninstall", about = "Remove a package from the project")]
    remove,
    
    #[command(about = "Scan project dependencies for known vulnerabilities")]
    scan,
    
    #[command(alias = "st", about = "Show statistics about the Kumo global cache and store")]
    stats,
    
    #[command(about = "Maintenance commands to clean cached files or dependencies")]
    prune,
    
    #[command(alias = "dr", about = "Run a health check on the store to detect corrupted files")]
    doctor,
    
    #[command(alias = "ex", about = "Explain why a package is present in the dependency tree")]
    explain,
    ...
    #[command(alias = "up", about = "Update project dependencies to their latest available versions")]
    upgrade,
    
    #[command(about = "Manage Kumo configuration and security policies")]
    config,
    
    #[command(alias = "tsx", about = "Execute TypeScript files via tsx or compile with tsc")]
    ts,
