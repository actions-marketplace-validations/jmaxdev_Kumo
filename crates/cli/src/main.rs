...
use resolver::{Resolver, Lockfile};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Install => {
            ...
            let config_content: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(config_path)?)?;
        ...
        Commands::Scan => {
            ...
            if total_vulns == 0 {
                println!("✅ Scan complete: No vulnerabilities found in {} packages.", lockfile.packages.len());
            } else {
                println!("⚠️ Scan complete: Found {} vulnerabilities across the dependency tree.", total_vulns);
            }
        }
        Commands::Stats => {
        ...
        Commands::Patch { name } => {
            ...
            println!("💡 Package extracted to .kumo/patch/{}. Edit and run 'kumo patch-commit'.", name);
        }
        Commands::Timeline => {
...
async fn resolve_and_install(
    ...
    println!("📦 Downloading {} packages in parallel...", lockfile.packages.len());
    
    let cpus = num_cpus::get();
    let concurrent_limit = cpus * 2;
    println!("🚀 Concurrency set to {} (based on {} CPU cores)", concurrent_limit, cpus);

    let multi_progress = indicatif::MultiProgress::new();
    let main_pb = multi_progress.add(indicatif::ProgressBar::new(lockfile.packages.len() as u64));
    main_pb.set_style(indicatif::ProgressStyle::with_template(
        "{spinner:.green} [{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} {msg}"
    ).unwrap().progress_chars("#>-"));
    main_pb.set_message("Installing packages...");

    let stream = futures::stream::iter(lockfile.packages.clone())
        .map(|(key, pkg)| {
            let store = store;
            ...
            async move {
                ...
                if let Some(file_map) = store.load_index(&key).await? {
                    ...
                    core::package::link_package(store, &target_dir, &file_map).await?;
                    pb.finish_and_clear();
                    main_pb.inc(1);
                ...
                let is_safe = security.validate_package(
                    name, 
                    version, 
                    None, 
                    false, 
                ...
                if let Some(bin) = pkg.metadata.bin.as_ref() {
...
async fn install_global(
    ...
    if let Some(bin) = metadata.bin {
        match bin {
            serde_json::Value::String(path) => {
                create_shim(&global_bin, &name, &global_node_modules.join(&name).join(path)).await?;
            }
            serde_json::Value::Object(map) => {
                for (cmd_name, path) in map {
                    if let Some(p) = path.as_str() {
                        create_shim(&global_bin, &cmd_name, &global_node_modules.join(&name).join(p)).await?;
                    }
                }
            }
            _ => {}
...
async fn run_script(name: &str, args: Vec<String>) -> Result<()> {
    ...
    let config_path = if pkg_json_path.exists() { pkg_json_path } else { npm_json_path };
    ...
    if let Some(script) = config_content.get("scripts").and_then(|s| s.get(name)).and_then(|v| v.as_str()) {
        println!("🚀 Running script: {} ({})", name, script);
        
        let mut child = std::process::Command::new("powershell")
            .arg("-Command")
            .arg(format!("{} {}", script, args.join(" ")))
            .env("PATH", format!("{}\\packages\\.bin;{}", std::env::current_dir()?.display(), std::env::var("PATH").unwrap_or_default()))
            .spawn()?;
...
async fn show_stats(store: &Store) -> Result<()> {
    ...
    println!("💾 Store Disk Usage: {:.2} MB", total_size as f64 / 1024.0 / 1024.0);
    println!("🚀 Estimated Space Saved: {:.2} MB", (total_size as f64 * 0.4) / 1024.0 / 1024.0); // Simple estimate based on deduplication
    Ok(())
}
...
async fn run_doctor(store: &Store) -> Result<()> {
    ...
    if corrupted > 0 {
        println!("⚠️ Found {} corrupted files. Run 'kumo repair' (future) to fix.", corrupted);
    } else {
...
async fn explain_package(name: &str) -> Result<()> {
    ...
    let mut found = false;
    for (key, pkg) in &lockfile.packages {
        if key.starts_with(name) && (key.chars().nth(name.len()) == Some('@') || key.len() == name.len()) {
            println!("📦 Found: {}", key);
            found = true;
